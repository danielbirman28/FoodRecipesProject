...
For our modeling algorithm, we used a <code class="language-plaintext highlighter-rouge">RandomForrestRegressor</code> and a <code class="language-plaintext highlighter-rouge">RandomizedSearchCV</code> to tune our hyperparameters of <code class="language-plaintext highlighter-rouge">'regressor__n_estimators'</code>, <code class="language-plaintext highlighter-rouge">'regressor__max_depth'</code>, <code class="language-plaintext highlighter-rouge">'regressor__min_samples_split'</code>, and <code class="language-plaintext highlighter-rouge">'regressor__min_samples_leaf'</code>. The reason why we chose these hyperparameters is because we wanted to control the variance and avoid overfitting of the training set that is likely for random forrests. We chose a Random Forest Regressor for the model because it effectively captures non-linear relationships, is robust to overfitting, and requires minimal data preprocessing. In the preprocessing stage, we transformed the data to prepare it for the regression model. Numeric features (<code class="language-plaintext highlighter-rouge">'minutes'</code>, <code class="language-plaintext highlighter-rouge">'tag_length'</code>, <code class="language-plaintext highlighter-rouge">'calories (#)'</code>, and <code class="language-plaintext highlighter-rouge">'n_ingredients'</code>) are scaled using <code class="language-plaintext highlighter-rouge">StandardScalar</code> and transformed using <code class="language-plaintext highlighter-rouge">QuantileTransformer</code> to ensure they follow a uniform distribution. For boolean columns (<code class="language-plaintext highlighter-rouge">'is_dessert'</code>, <code class="language-plaintext highlighter-rouge">'is_healthy'</code>), a <code class="language-plaintext highlighter-rouge">FunctionTransformer</code> is used to select them without any additional transformation. These preprocessing steps are combined using a <code class="language-plaintext highlighter-rouge">ColumnTransformer</code>, ensuring that both numeric and boolean features are appropriately processed before being passed to the Random Forest Regressor.
...
Here is a graph that depicts how are model is trained on the data:

<iframe
  src="assets/final-model.html"
  width="800"
  height="600"
  frameborder="0"
></iframe>
